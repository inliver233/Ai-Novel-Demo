...
test("ui: style profiles create and set project default", async ({ page, request }) => {
  ...
  await page.getByLabel("style_name", { exact: true }).fill("E2E 自定义风格");
  ...
  await expect(page.getByText("E2E 自定义风格")).toBeVisible();

  await page.getByRole("button", { name: "设为默认:E2E 自定义风格", exact: true }).click();
  await expect(page.getByLabel("project_default_style", { exact: true })).toContainText("E2E 自定义风格");
