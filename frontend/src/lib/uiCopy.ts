export const UI_COPY = {
  ...
  writing: {
    ...
    memoryUpdateDrawer: {
      ...
      subtitle: "提议（Propose）→ 审核（Review）→ 应用（Apply）",
      step1: "步骤 1：提议（Propose）",
      step2: "步骤 2：审核（Review）",
      step3: "步骤 3：应用（Apply）",
      inputTitle: "输入（memory_update_v1）",
      inputHint: "支持：ops 数组 或包含 ops 字段的对象。",
      focusLabel: "focus（可选）",
      focusPlaceholder: "例如：只更新角色关系 / 仅新增事件",
      jsonLabel: "memory_update_json",
      autoPropose: "一键生成提议（Auto Propose）",
      propose: "生成提议（Propose）",
      proposing: "生成中...（Proposing...）",
      proposeFailed: "提议（Propose）失败",
      reviewTitle: "提议差异（Diff）",
      accept: "接受（Accept）",
      diffPreview: "差异预览（Diff Preview）",
      before: "变更前（Before）",
      after: "变更后（After）",
      applyAccepted: "应用已接受项（Apply）",
      applying: "应用中...（Applying...）",
      applyFailed: "应用（Apply）失败",
      retryApply: "重试应用（Retry Apply）",
      applyResultTitle: "应用结果（Apply Result）",
      warnings: "警告（Warnings）",
      warningsZero: "警告（Warnings）：0",
      missingProposeHint: "请先在“步骤 1”生成提议。",
    },
    foreshadowDrawer: {
      title: "伏笔面板",
      openCountLabel: "开放伏笔（Open Loops）",
      openOnlyHint: "说明：这里仅展示未回收的伏笔；标记回收后会从列表移除。",
      hasMoreTag: "（has_more）",
      requestIdPrefix: "request_id:",
      refresh: "刷新",
      refreshing: "加载中...",
      filterLabel: "筛选（chapter_id / title / preview）",
      filterPlaceholder: "输入关键词，例如：chapter_id / 角色名",
      empty: "暂无开放伏笔（Open Loops）。请先在“章节分析”里保存到记忆库。",
      noTitle: "（无标题）",
      metaChapterId: "章节（chapter_id）",
      metaScore: "重要性（score）",
      metaTimeline: "时间线（timeline）",
      jumpChapter: "跳转章节",
      annotatePage: "标注页",
      resolve: "标记回收",
