...
对照来源：`frontend/src/App.tsx` 路由清单（以实际代码为准）。
...
- 高级调试：默认折叠；并可通过“显示高级调试”开关整体隐藏/显示（后续 IA-003 落地）
...
- **预览**：`preview`
- **导出**：`export`
- （可选/后续）**章节分析**：`chapter-analysis`（若定位为“查看/复盘”，放在本组；否则可归入“项目工作台”）
...
- 2026-01-21：IA-002 已在 `frontend/src/components/layout/AppShell.tsx` 落地分组与 `rag` 入口；高级调试“整体显示/隐藏开关”留给 IA-003。
